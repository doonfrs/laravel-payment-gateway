        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('payment-gateway::messages.order_details') }}</h2>
                        <div class="space-y-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('payment-gateway::messages.customer_information') }}</h2>
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-600">{{ __('payment-gateway::messages.customer_phone') }}:</span>
                ...
                <!-- Payment Methods -->
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 mb-6">{{ __('payment-gateway::messages.select_payment_method') }}</h2>
                    ...
                    @if($errors->any())
                        <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-red-800">{{ __('payment-gateway::messages.errors_with_submission') }}</h3>
                    ...
                    <form method="POST" action="{{ route('payment-gateway.process', ['order' => $paymentOrder->order_code]) }}" id="payment-form">
                        @csrf
                        <input type="hidden" name="payment_method_id" id="selected-method" required>

                        <div class="grid md:grid-cols-2 gap-4 mb-8">
                            @forelse($paymentMethods as $method)
                                <div class="payment-method-card border-2 border-gray-200 rounded-lg p-6 cursor-pointer hover:shadow-md" 
                                    ...
                                    <div class="text-center">
                                        ...
                                        @else
                                            <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg mx-auto mb-4 flex items-center justify-center">
                                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                                                </svg>
                                            </div>
                                        @endif
                                        <h3 class="font-semibold text-gray-900 mb-2">{{ $method->display_name ?: $method->name }}</h3>
                                ...
                                <div class="col-span-2">
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                        <div class="flex">
                                            ...
                                            <div class="ml-3">
                                                <h3 class="text-sm font-medium text-yellow-800">{{ __('payment-gateway::messages.no_payment_methods_available') }}</h3>
                                                <p class="mt-1 text-sm text-yellow-700">{{ __('payment-gateway::messages.contact_support_assistance') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
